...
exports.listPublicIngredientes = async (req, res) => {
  try {
    console.log("Listando todos los ingredientes principales...");

    // Realizar la consulta para obtener todos los ingredientes principales
    const ingredientes = await ingredientesPrincipales.findAll();

    console.log("Ingredientes recuperados:", ingredientes);

    res.status(200).json({
      datos: ingredientes,
      mensaje: "Lista de todos los ingredientes recuperada correctamente",
    });
  } catch (error) {
